...
void mqtt_publish_data(char *pub_payload)
{
	...
	esp_mqtt_client_publish(client, pub_topic, pub_payload, strlen(pub_payload), qos, retain);
}
/**
 ...
 * @return: 
 */
void TaskMqtt_Up_Data(void *pvParameters)
{
	while(1)
	{
		//post_data_to_clouds();
